...
impl PageEndpoint {
    ...
    #[turbo_tasks::function]
    async fn compute_internal_ssr_chunk_module(
        self: Vc<Self>,
    ) -> Result<Vc<InternalSsrChunkModule>> {
        let this = self.await?;
    ...
    async fn internal_ssr_chunk(
        ...
        async move {
            ...
            let InternalSsrChunkModule {
                ssr_module,
                runtime,
            } = *self.compute_internal_ssr_chunk_module().await?;
...
impl Endpoint for PageEndpoint {
    ...
    async fn root_modules(self: Vc<Self>) -> Result<Vc<Modules>> {
        ...
        let ssr_chunk_module = self.compute_internal_ssr_chunk_module().await?;
